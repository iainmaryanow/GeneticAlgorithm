...
class GeneticAlgorithm:
  def __init__(self, fitness_fn, number_of_individuals=20, number_of_genes=10, mutation_rate=.25, mutation_deviation=1):
  ...
  def evolve(self, number_of_generations=5):
  ...
  def _initialize_population(self, number_of_individuals):
    ...
    for _ in range(number_of_individuals):
      genes = [randint(0, 9) for _ in range(self._number_of_genes)]
  ...
  def _create_child(self, parent_1, parent_2):
    ...
    if random() <= self._mutation_rate:
      ...
      allele = round(gauss(genes[gene_index], self._mutation_deviation))
      genes[gene_index] = allele
